...
public class cnCoarseNbhood {
	...
	static int VL=2; // Verbosity Level : 0=silent ... 3=very talkative 
	...
	public static void main(String[] args) {
	    ...
	    if (args.length<2){
			System.err.println("Coarse NBHood : graph_id and residue-nr. need to be given .... i.e. 7 110");
			System.exit(1);
		}		
		...
		try {
			...
			System.out.println("Coarse Scoring Target neighborhoods v.0.5. "); 
...
//			retrieve the original nbhood into orig_shell
			...
			mstmt = conn.createStatement();
			mstmt.executeUpdate("create table orig_shell as " +
					"select graph_id,i_node_id,i_cid,i_num,i_res,i_sstype,j_node_id,j_cid,j_num,j_res,j_sstype,BB,SC, 1 as shell " +
					"from "+targetEdges+
					" where graph_id="+graphid+" and i_num="+resnr+";");
			mstmt.close(); 
	
			System.out.println("adding the original 2nd shell ...");  
			sql = "select j_cid, j_num, j_res, j_sstype, BB, SC from orig_shell where shell=1 and graph_id="+graphid+" and i_num="+resnr+";";
			mstmt = conn.createStatement();
			mrsst = mstmt.executeQuery(sql);
			i=0; 
			while (mrsst.next()) {
				i++; 
				oj_num = mrsst.getInt(2);
				System.out.print(i+":"+mrsst.getString(1));
				System.out.print("\t"+oj_num); 
				System.out.print("\t"+mrsst.getString(3)); 
				System.out.print("\t"+mrsst.getString(4));
				System.out.print("\t"+mrsst.getInt(5)); 
				System.out.print("\t"+mrsst.getInt(6)); 
				System.out.println(); 
				mjst = conn.createStatement();
				sql = "insert into orig_shell select graph_id,i_node_id,i_cid,i_num,i_res,i_sstype,j_node_id,j_cid,j_num,j_res,j_sstype,BB,SC, 2 as shell " +
						"from "+targetEdges+" " +
						"where graph_id="+graphid+" and i_num="+oj_num+";";
			...
			System.out.println("gathering all direct and indirect neighbours.");  
			sql = "select j_num, j_res, j_sstype, min(shell) as shell, count(*) as cn " +
					"from orig_shell where j_num!="+resnr+" group by j_num order by j_num;";
			mstmt = conn.createStatement();
			mrsst = mstmt.executeQuery(sql);
			o1=0; 
			o2=0; 		
...
//							// only once for building the sidelines
	...
	public static void getEntropy( String nbs, String centRes, String predec) { // pass centSS as well ??? 
		...
		try {
			...
			this_n = nbs.replace("%","_"); 
			prec_n = predec.replace("%","_");
			...
			if (prec_n.equals(this_n)) {
				...
				sql = "create table IF NOT EXISTS nbhashtables."+prec_n+" as select res, n, k from nbstrings where nwg like '%"+nbs+"%';"; 
