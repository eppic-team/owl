...
public class Individuals {//extends HashSet<Pair<Integer>> {
	...
	protected static final String[] file_extension = {"indi","ust","cmap","graph"};
	...
	public void setIndis (Individuals in, HashSet<Pair<Integer>> contactset) throws PdbCodeNotFoundError, SQLException, PdbLoadError{
		...
		Bound[][] bound = Reconstructer.convertRIGraphToBoundsMatrix(this.getSubRIGraph());//this.convertDistanceMatrixToBounds();//Reconstructer.convertRIGraphToBoundsMatrix(rig);
		...
		for(int i = 0; i < length - 1; i++){						//loop for conversion
			...
			for(int j = i + 1; j < length; j++){
				if(input[i][j] != null && counter < numconts){		//check point to make sure no null contacts are
					...
					if(i != j - 1){									//check point to make sure no next neighbour contacts
																	//are taken
	 ...
	 * whether or not the number of entries does not exceed the <code> numconts </code>.
	 * <p>
	 * The only difference to the method <code> setEntries (Bound[][], int, boolean) </code> is that next neighbour contacts (i.e.
	...
	public void setIndis (RIGraph rig) throws PdbCodeNotFoundError, SQLException, PdbLoadError {
		...
		this.setName(rig.getPdbCode());											//PDB code is used as the 'name' of this instance
		
		Bound[][] bound = Reconstructer.convertRIGraphToBoundsMatrix(rig);//convertDistanceMatrixToBounds(fulldistancematrix);//Reconstructer.convertRIGraphToBoundsMatrix(rig);
	...
	public void setIndis(String file_path) throws IOException, SQLException, PdbCodeNotFoundError, PdbLoadError{
		...
		if(file.exists()){
			...
			if(isReadableFileFormat(file)){
				...
				if(this.name == null){
					//exits, if the file did not contain any PDB code,
				...
				if(this.store.size() == 0){
					//exits, if no contact was read from the file
					System.err.println("This '"+filetype+"' file in the directory '"+file_path+"' did not contain"+
				...
				//exits, if the file is neither a .cmap or a .graph file
	...
	public void setCMError (Bound[][] bound, RIGraph rig) throws PdbCodeNotFoundError, SQLException, PdbLoadError {
		this.CMError = Scorer.getCMError(bound,rig);//Scorer.getCMError(rig,bound);
	}
	...
	public void printToFile() throws IOException, FileNotFoundException {
		String st = "#CMVIEW GRAPH FILE ver: 1.0\n#SEQUENCE: "+this.getSequence()+"\n"+
		...
		int dim = this.entries.length;
		for(int i = 0; i < dim; i++){
			st = st + this.entries[i][0] + "\t" + this.entries[i][1] + 1.0 + "\n";
		}
	...
	public void printToFile(String dir, String addname1, String addname2) throws IOException, FileNotFoundException {
		...
		for(int i = 0; i < dim; i++){
			st = st + contact_array[0][i] + "\t" + contact_array[1][i] + "\t" + 1.0 + "\n";
		}
