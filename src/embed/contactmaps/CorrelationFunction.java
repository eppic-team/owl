...
public class CorrelationFunction {
	 ...
	 * @throws PdbLoadError
	 */
	public static Population1[] evolve (Population1 starter, int generations) throws SQLException, PdbCodeNotFoundError, PdbLoadError{
		Population1[] pop = new Population1[generations];
		Population1 start = new Population1(starter);
		for(int i = 0; i <= generations - 1; i++){
			if(i == 0){
				pop[i] = new Population1(Population1.evolve(false, start));
			}
			else{
				pop[i] = new Population1(Population1.evolve(false, pop[i-1]));
			}
		}
		return pop;
	}
	 ...
	 */
	public static void printToFile (Population1[] pops) throws FileNotFoundException{
		int dim = pops.length;
	 ...
	 * @return st
	 */
	public static String toString(Population1[] pops){
		...
		for(int k = 0; k < 1; k++){
			...
			int j = 1;
				while(j <= numconts){
					...
					Population1 start = new Population1(off);
					Population1[] pop1 = evolve(start,10);
					for(int i = 0; i < 10; i++){
